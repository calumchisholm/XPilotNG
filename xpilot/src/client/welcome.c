...
/*
 * States a connection tso a meta server can be in.
 */
enum MetaState {
    MetaConnecting = 0,
    MetaReadable = 1,
...
static int Welcome_create_label(int position, const char *label_text)
{
	...
	Widget_create_label(subform_widget,
			    label_x, label_y,
			    label_width, label_height, 0, label_text);
...
static int Localnet_cb(int widget, void *user_data, const char **text)
{
    ...
    label = Welcome_create_label(
	1, "Searching for XPilot servers on your local network...");
    Widget_get_dimensions(subform_widget, &subform_width, &subform_height);
    ...
    Widget_destroy_children(subform_widget);
    if (!n)
	Welcome_create_label(
	    1, "No servers were found on your local network.");
    else
	Welcome_create_label(
	    0, "The following local XPilot servers were found:");
...
static void Ping_servers(void)
{
    ...
    sprintf(buf, "Pinging servers (%d seconds)...",
	    (maxwait + 500) / 1000);
    Welcome_create_label(1, buf);
...
static int Get_meta_data(void)
{
    ...
    if (!connections) {
	Welcome_create_label(
	    1, "Could not establish connections with any metaserver");
	return -1;
    }
				   ...
				   const char **text)
{
    ...
    Widget_create_activate(subform_widget,
			   label_x + label_width + 2 *(data_label_width), 
			   labels[i].yoff,
			   data_label_width, labels[i].height,
	...
	Widget_create_label(subform_widget,
			    label_x + label_width,
			    label_y,
			    player_label_width, label_height,
			    label_border, p);
...
static int Welcome_show_server_list(Connect_param_t * conpar)
{
    ...
    const int max_map_length = 50;
    const int max_version_length = 11;
    const int max_server_length = 50;
    ...
    int stat_width = XTextWidth(textFont, "Status", 8)
	+ extra_width + 2 * border;
    ...
    server_width =
	MIN(server_width,
	    subform_width - server_offset - space_width - stat_width -
	    space_width - server_border_width);
    ...
    Widget_create_label(subform_widget,
			queue_offset, yoff,
			queue_width, label_height, border, queue_header);

    Widget_create_label(subform_widget,
			bases_offset, yoff,
			bases_width, label_height, border, bases_header);

    Widget_create_label(subform_widget,
			team_offset, yoff,
			team_width, label_height, border, team_header);

    Widget_create_label(subform_widget,
			fps_offset, yoff,
			fps_width, label_height, border, fps_header);

    Widget_create_label(subform_widget,
			status_offset, yoff,
			status_width, label_height, border, status_header);
    ...
    Widget_create_label(subform_widget,
			map_offset, yoff,
			map_width, label_height, border, map_header);

    Widget_create_label(subform_widget,
			server_offset, yoff,
			server_width + server_border_width, label_height,
			border, server_header);

    Widget_create_label(subform_widget,
			ping_offset, yoff,
			ping_width, label_height, border, ping_header);

    Widget_create_label(subform_widget,
			stat_offset, yoff,
			stat_width, label_height, border, stat_header);
	...
	Widget_create_label(subform_widget, fps_offset, yoff, fps_width,
			    label_height, border, sip->fps_str);
	...
	Widget_create_label(subform_widget,
			    map_offset, yoff,
			    map_width, label_height, border, sip->mapname);
	Widget_create_activate(subform_widget,
			       server_offset,
			       yoff - (border == 0),
    ...
    int pingw_width = XTextWidth(buttonFont,
				 ping_text,
				 strlen(ping_text))
      + extra_width + 2 * first_border;
    ...
    if (!next_page_widget)
      next_page_widget = Widget_create_activate(form_widget,
						(int) (2*(top_width / 4)+next_width),
						top_height - (next_height) - space_height,
						next_width, next_height,
						next_border, next_text,
						Internet_next_page_cb, (void *) conpar);   
    if (!first_page_widget)
      first_page_widget = Widget_create_activate(form_widget,
						 (int) (2*(top_width / 4)),
						 ...
						 (void *) conpar);
    if (!ping_servers_widget)
      ping_servers_widget = Widget_create_activate(form_widget,
						   next_height,
						   top_height - (next_height) - space_height,
