...
void CON_UpdateConsole(ConsoleInformation *console) {
	...
	SDL_FillRect(console->ConsoleSurface, NULL, SDL_MapRGBA(console->ConsoleSurface->format, 0, 20, 0, SDL_ALPHA_OPAQUE));
...
ConsoleInformation *CON_Init(const char *FontName, SDL_Surface *DisplayScreen, int lines, SDL_Rect rect) {
	...
	SDL_FillRect(newinfo->ConsoleSurface, NULL, SDL_MapRGBA(newinfo->ConsoleSurface->format, 0, 20, 0, newinfo->ConsoleAlpha));
	...
	//SDL_FreeSurface(Temp);
	SDL_FillRect(newinfo->InputBackground, NULL, SDL_MapRGBA(newinfo->ConsoleSurface->format, 0, 20, 0, SDL_ALPHA_OPAQUE));
