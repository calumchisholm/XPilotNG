...
static void Transport_to_home(int ind)
{
	...
	bx = World.check[check].x;
	by = World.check[check].y;
    } else {
	bx = World.base[pl->home_base].pos.x;
	by = World.base[pl->home_base].pos.y;
    }
    dx = CENTER_XCLICK(bx - pl->pos.cx);
    dy = CENTER_YCLICK(by - pl->pos.cy);
    t = pl->count + 0.5f;
    if (2 * t <= T) {
	m = 2 / t;
    } else {
...
void Emergency_shield (int ind, int on)
{
	...
	if (!BIT(pl->used, OBJ_EMERGENCY_SHIELD)) {
	    ...
	    sound_play_sensors(pl->pos.cx, pl->pos.cy, EMERGENCY_SHIELD_ON_SOUND);
	}
    } else {
	if (pl->emergency_shield_left <= 0) {
	...
	if (BIT(pl->used, OBJ_EMERGENCY_SHIELD)) {
	    ...
	    sound_play_sensors(pl->pos.cx, pl->pos.cy, EMERGENCY_SHIELD_OFF_SOUND);
	}
    }
}
...
#define UPDATE_RATE 100
	...
	if (BIT(pl->used, OBJ_REPAIR)) {
	    ...
	    int x = (targ->pos.x + 0.5) * BLOCK_CLICKS;
	    int y = (targ->pos.y + 0.5) * BLOCK_CLICKS;
	    if (Wrap_length(pl->pos.cx - x, pl->pos.cy - y) > 90.0 * CLICK
...
#define RANDOM_REAR_WORM
		...
		for (k = 0; k < NumObjs; k++) {
			...
			position ballpos;
			ballpos.x = b->pos.cx + (PIXEL_TO_CLICK(w.x) - pl->pos.cx);
			ballpos.y = b->pos.cy + (PIXEL_TO_CLICK(w.y) - pl->pos.cy);
			ballpos.x = WRAP_XCLICK(ballpos.x);
			ballpos.y = WRAP_YCLICK(ballpos.y);
			Object_position_init_clicks(b, ballpos.x, ballpos.y);
	    ...
	    pl->wormHoleDest = j;
	    Player_position_init_clicks(pl, PIXEL_TO_CLICK(w.x), PIXEL_TO_CLICK(w.y));
	...
	if (BIT(pl->lock.tagged, LOCK_PLAYER)) {
		...
		Wrap_length(pl->pos.cx - Players[GetInd[pl->lock.pl_id]]->pos.cx,
			     pl->pos.cy - Players[GetInd[pl->lock.pl_id]]->pos.cy) / CLICK;
	}
    }
