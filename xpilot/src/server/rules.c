...
uint32_t	DEF_HAVE = (HAS_SHIELD|HAS_SHOT|HAS_LASER);
uint32_t	DEF_USED = (HAS_SHIELD|USES_COMPASS);
uint32_t	USED_KILL =
	(USES_REFUEL|USES_REPAIR|USES_CONNECTOR|HAS_SHOT|HAS_LASER
	|USES_TRACTOR_BEAM|USES_CLOAKING_DEVICE|USES_PHASING_DEVICE
	|USES_DEFLECTOR|HAS_EMERGENCY_SHIELD|USES_EMERGENCY_THRUST);
