...
#define GRID_RESET()	(memset(grid.pt, 0, sizeof grid.pt), \
    ...
    i = sizeof(ipos) * RES;
    if (!(w->pts[0] = (ipos*)malloc(w->num_points * i))
	|| (w->num_l_gun
	    && !(w->l_gun[0] = (ipos*)malloc(w->num_l_gun * i)))
	|| (w->num_r_gun
	    && !(w->r_gun[0] = (ipos*)malloc(w->num_r_gun * i)))
	|| (w->num_l_rgun
	    && !(w->l_rgun[0] = (ipos*)malloc(w->num_l_rgun * i)))
	|| (w->num_r_rgun
	    && !(w->r_rgun[0] = (ipos*)malloc(w->num_r_rgun * i)))
	|| (w->num_l_light
	    && !(w->l_light[0] = (ipos*)malloc(w->num_l_light * i)))
	|| (w->num_r_light
	    && !(w->r_light[0] = (ipos*)malloc(w->num_r_light * i)))
	|| (w->num_m_rack
	    && !(w->m_rack[0] = (ipos*)malloc(w->num_m_rack * i)))
	) {
	warn("Not enough memory for ship shape");
	if (w->pts[0]) {
	    free(w->pts[0]);
	    if (w->l_gun[0]) {
		free(w->l_gun[0]);
			   ...
			   unsigned shape_version)
{
	...
	for (i = 0; i < w->num_points && i < MAX_SHIP_PTS; i++) {
	    sprintf(&buf[buflen], " %d,%d",
		    (int)w->pts[i][0].x/64, (int)w->pts[i][0].y/64);
	    buflen += strlen(&buf[buflen]);
	}
	sprintf(&buf[buflen], ")(EN: %d,%d)(MG: %d,%d)",
		(int)w->engine[0].x/64, (int)w->engine[0].y/64,
		(int)w->m_gun[0].x/64, (int)w->m_gun[0].y/64);
	buflen += strlen(&buf[buflen]);
	...
	if (w->num_l_gun > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->l_gun[i][0].x/64, (int)w->l_gun[i][0].y/64);
	...
	if (w->num_r_gun > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->r_gun[i][0].x/64, (int)w->r_gun[i][0].y/64);
		tmplen += strlen(&tmp[tmplen]);
	    }
	    strcpy(&tmp[tmplen], ")");
	    tmplen++;
	...
	if (w->num_l_rgun > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->l_rgun[i][0].x/64, (int)w->l_rgun[i][0].y/64);
		tmplen += strlen(&tmp[tmplen]);
	    }
	    strcpy(&tmp[tmplen], ")");
	    tmplen++;
	...
	if (w->num_r_rgun > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->r_rgun[i][0].x/64, (int)w->r_rgun[i][0].y/64);
		tmplen += strlen(&tmp[tmplen]);
	    }
	    strcpy(&tmp[tmplen], ")");
	    tmplen++;
	...
	if (w->num_l_light > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->l_light[i][0].x/64, (int)w->l_light[i][0].y/64);
		tmplen += strlen(&tmp[tmplen]);
	    }
	    strcpy(&tmp[tmplen], ")");
	    tmplen++;
	...
	if (w->num_r_light > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->r_light[i][0].x/64, (int)w->r_light[i][0].y/64);
		tmplen += strlen(&tmp[tmplen]);
	    }
	    strcpy(&tmp[tmplen], ")");
	    tmplen++;
	...
	if (w->num_m_rack > 0) {
		...
		sprintf(&tmp[tmplen], " %d,%d",
			(int)w->m_rack[i][0].x/64, (int)w->m_rack[i][0].y/64);
		tmplen += strlen(&tmp[tmplen]);
	    }
	    strcpy(&tmp[tmplen], ")");
	    tmplen++;
