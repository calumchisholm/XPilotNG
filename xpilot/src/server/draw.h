...
typedef struct {			/* Defines wire-obj, i.e. ship */
    ipos	*pts[MAX_SHIP_PTS];	/* the shape rotated many ways */
    int		num_points;		/* total points in object */
    ipos	engine[RES];		/* Engine position */
    ipos	m_gun[RES];		/* Main gun position */
    int		num_l_gun,
		num_r_gun,
		num_l_rgun,
		num_r_rgun;		/* number of additional cannons */
    ipos	*l_gun[MAX_GUN_PTS],	/* Additional cannon positions, left*/
    ...
    int		num_l_light,		/* Number of lights */
		num_r_light;
    ipos	*l_light[MAX_LIGHT_PTS], /* Left and right light positions */
		*r_light[MAX_LIGHT_PTS];
    int		num_m_rack;		/* Number of missile racks */
    ipos	*m_rack[MAX_RACK_PTS];
