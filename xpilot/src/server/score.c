    ...
    switch(st) {
		...
		if (i_tank_owner == GetInd(victim->id))
		    ...
		    * options.tankKillScoreMult;
    	    ...
    	    Score_players(true_killer, sc, victim->name, victim, -sc, killer->name, true);
	    break;
		    ...
		    * options.unownedKillScoreMult;
	    	...
	    	if (killer == victim){
		    	...
		    	* options.selfKillScoreMult;
	    	    ...
	    	    sc = Rate( Get_Score(killer),  Get_Score(victim)) * options.ballKillScoreMult;
	    	    Score_players(killer, sc, victim->name, victim, -sc, killer->name, true);
		}
	    }
	    break;
    	case SCORE_HIT_MINE:
    	    sc = Rate( Get_Score(killer),  Get_Score(victim)) * options.mineScoreMult;
    	    Score_players(killer, sc, victim->name, victim, -sc, killer->name, false);
	    break;
	    ...
	    sc = Rate( Get_Score(killer), ASTEROID_SCORE) * options.unownedKillScoreMult;
	    if (!options.zeroSumScoring) Score(killer, sc, ((wireobject_t *)extra)->pos, "");
	    break;
    	case SCORE_ASTEROID_DEATH:
	    sc = Rate(0.0,  Get_Score(victim)) * options.unownedKillScoreMult;
	    if (!options.zeroSumScoring) Score(victim, -sc, victim->pos, "[Asteroid]");
	    break;
		...
		if (killer->id == victim->id) {
		    sc = Rate(0.0,  Get_Score(victim)) * options.selfKillScoreMult;
		} else {
		...
		if (victim->id == killer->id) {
		    ...
		    sc = Rate( Get_Score(killer),  Get_Score(victim)) * options.laserKillScoreMult;
		    Score_players(killer, sc, victim->name, victim, -sc, killer->name, true);
		...
		if (!options.zeroSumScoring) Score(victim, -sc, victim->pos, "");
    	...
    	case SCORE_SELF_DESTRUCT:
    	    if (options.selfDestructScoreMult != 0) {
    	    	sc = Rate(0.0,  Get_Score(killer)) * options.selfDestructScoreMult;
    	    	if (!options.zeroSumScoring) Score(killer, -sc, killer->pos, "Self-Destruct");
    	    }
	    break;
